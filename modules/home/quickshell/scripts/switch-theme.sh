...
# ── Zed ─────────────────────────────────────────────────────────────────────
switch_zed() {
  local theme_dir="$HOME/.config/zed/themes"
  ...
  local sk_kw sk_str sk_fn sk_type sk_var sk_cmt sk_op sk_num
  local sk_const sk_tag sk_attr sk_prop sk_preproc sk_lbl sk_punc
  local sk_esc sk_spec sk_fn_weight

  if [ "$THEME" = "carbonfox" ]; then
    sk_kw="#78a9ff"      sk_str="#be95ff"    sk_fn="#ff7eb6"
    sk_type="#78a9ff"    sk_var="#dde1e6"    sk_cmt="#525252"
    sk_op="#78a9ff"      sk_num="#82cfff"    sk_const="#3ddbd9"
    sk_tag="#ee5396"     sk_attr="#3ddbd9"   sk_prop="#dde1e6"
    sk_preproc="#ee5396" sk_lbl="#3ddbd9"    sk_punc="#f2f4f8"
    sk_esc="#ff7eb6"     sk_spec="#3ddbd9"   sk_fn_weight=700
  else
    sk_kw="${C[mauve]}"    sk_str="${C[green]}"   sk_fn="${C[blue]}"
    sk_type="${C[pink]}"   sk_var="${C[text]}"    sk_cmt="${C[overlay0]}"
    sk_op="${C[sky]}"      sk_num="${C[peach]}"   sk_const="${C[peach]}"
    sk_tag="${C[red]}"     sk_attr="${C[teal]}"   sk_prop="${C[text]}"
    sk_preproc="${C[red]}" sk_lbl="${C[teal]}"    sk_punc="${C[text]}"
    sk_esc="${C[peach]}"   sk_spec="${C[teal]}"   sk_fn_weight=null
  fi

  local theme_name="Quickshell ${C[name]}"

  cat > "$theme_dir/quickshell.json" << EOF
{
  ...
  "themes": [
    {
      ...
      "style": {
        ...
        "scrollbar.thumb.background": "${C[surface1]}",
        "scrollbar.thumb.hover_background": "${C[surface2]}",
        "scrollbar.thumb.border": "${C[surface0]}",
        "status_bar.background": "${C[crust]}",
        "title_bar.background": "${C[crust]}",
        "title_bar.inactive_background": "${C[mantle]}",
        "toolbar.background": "${C[mantle]}",
        "tab_bar.background": "${C[crust]}",
        "tab.inactive_background": "${C[crust]}",
        "tab.active_background": "${C[base]}",
        "search.match_background": "${C[surface0]}",
        "drop_target.background": "${C[surface0]}",
        "link_text.hover": "${C[blue]}",
        "text": "${C[text]}",
        "text.muted": "${C[subtext0]}",
        "text.placeholder": "${C[overlay0]}",
        "text.disabled": "${C[overlay0]}",
        "text.accent": "${C[accent]}",
        "icon": "${C[text]}",
        "icon.muted": "${C[subtext0]}",
        "icon.disabled": "${C[overlay0]}",
        "icon.placeholder": "${C[overlay0]}",
        "icon.accent": "${C[accent]}",
        "ghost_element.background": "#00000000",
        "ghost_element.hover": "${C[surface0]}",
        "ghost_element.active": "${C[surface1]}",
        "ghost_element.selected": "${C[surface0]}",
        "ghost_element.disabled": "#00000000",
        "element.background": "#00000000",
        "element.hover": "${C[surface0]}",
        "element.active": "${C[surface1]}",
        "element.selected": "${C[surface0]}",
        "element.disabled": "#00000000",
        "error": "${C[red]}",
        "error.background": "${C[red]}20",
        "error.border": "${C[red]}",
        "warning": "${C[yellow]}",
        "warning.background": "${C[yellow]}20",
        "warning.border": "${C[yellow]}",
        "info": "${C[blue]}",
        "info.background": "${C[blue]}20",
        "info.border": "${C[blue]}",
        "hint": "${C[teal]}",
        "hint.background": "${C[teal]}20",
        "hint.border": "${C[teal]}",
        "success": "${C[green]}",
        "success.background": "${C[green]}20",
        "success.border": "${C[green]}",
        "conflict": "${C[peach]}",
        "conflict.background": "${C[peach]}20",
        "conflict.border": "${C[peach]}",
        "modified": "${C[yellow]}",
        "modified.background": "${C[yellow]}20",
        "modified.border": "${C[yellow]}",
        "created": "${C[green]}",
        "created.background": "${C[green]}20",
        "created.border": "${C[green]}",
        "deleted": "${C[red]}",
        "deleted.background": "${C[red]}20",
        "deleted.border": "${C[red]}",
        "renamed": "${C[sapphire]}",
        "renamed.background": "${C[sapphire]}20",
        ...
        "predictive": "${C[overlay1]}",
        "predictive.background": "#00000000",
        "predictive.border": "#00000000",
        "unreachable": "${C[surface2]}",
        "unreachable.background": "#00000000",
        "unreachable.border": "#00000000",
        "editor.background": "${C[base]}",
        "editor.foreground": "${C[text]}",
        "editor.gutter.background": "${C[base]}",
        "editor.invisible": "${C[surface1]}",
        "editor.wrap_guide": "${C[surface0]}",
        "editor.active_wrap_guide": "${C[surface1]}",
        "editor.line_number": "${C[overlay0]}",
        "editor.active_line_number": "${C[text]}",
        "editor.highlighted_line.background": "${C[surface0]}",
        "editor.active_line.background": "${C[mantle]}",
        "editor.selection.background": "${C[surface1]}",
        "editor.selection.border": null,
        "editor.active_selection.background": "${C[surface2]}",
        "editor.find_match.background": "${C[yellow]}50",
        "editor.find_match_highlight.background": "${C[peach]}30",
        "editor.find_all_matches.background": "${C[surface0]}",
        "editor.occurrence_highlight.background": "${C[surface0]}",
        "editor.bracket_matching.background": "${C[surface0]}",
        "editor.bracket_matching.border": "${C[accent]}",
        "editor.document_highlight.read_background": "${C[surface0]}",
        "editor.document_highlight.write_background": "${C[surface1]}",
        "editor.indent_guide": "${C[surface0]}",
        "editor.indent_guide_active": "${C[surface1]}",
        "terminal.background": "${C[base]}",
        "terminal.foreground": "${C[text]}",
        "terminal.bright_foreground": "${C[text]}",
        "terminal.dim_foreground": "${C[subtext0]}",
        "terminal.ansi.black": "${C[surface1]}",
        ...
        "terminal.ansi.white": "${C[subtext1]}",
        ...
        "terminal.ansi.bright_magenta": "${C[pink]}",
        "terminal.ansi.bright_cyan": "${C[teal]}",
        "terminal.ansi.bright_white": "${C[subtext0]}",
        "terminal.ansi.dim_black": "${C[surface0]}",
        "terminal.ansi.dim_red": "${C[maroon]}",
        "terminal.ansi.dim_green": "${C[green]}",
        "terminal.ansi.dim_yellow": "${C[yellow]}",
        "terminal.ansi.dim_blue": "${C[sapphire]}",
        "terminal.ansi.dim_magenta": "${C[mauve]}",
        "terminal.ansi.dim_cyan": "${C[sky]}",
        "terminal.ansi.dim_white": "${C[overlay1]}",
        "syntax": {
          "attribute": { "color": "$sk_attr" },
          "boolean": { "color": "$sk_kw" },
          "comment": { "color": "$sk_cmt", "font_style": "italic" },
          "comment.doc": { "color": "$sk_cmt", "font_style": "italic" },
          "constant": { "color": "$sk_const" },
          "constructor": { "color": "$sk_fn" },
          "embedded": { "color": "$sk_spec" },
          "emphasis": { "font_style": "italic" },
          "emphasis.strong": { "font_weight": 700 },
          "enum": { "color": "$sk_type" },
          "function": { "color": "$sk_fn", "font_weight": $sk_fn_weight },
          "function.method": { "color": "$sk_fn", "font_weight": $sk_fn_weight },
          "function.builtin": { "color": "$sk_const" },
          "function.special": { "color": "$sk_spec" },
          "keyword": { "color": "$sk_kw" },
          "keyword.directive": { "color": "$sk_preproc" },
          "keyword.return": { "color": "$sk_kw" },
          "label": { "color": "$sk_lbl" },
          "link_text": { "color": "${C[blue]}" },
          "link_uri": { "color": "${C[teal]}" },
          "number": { "color": "$sk_num" },
          "operator": { "color": "$sk_op" },
          "predictive": { "color": "${C[overlay0]}", "font_style": "italic" },
          "preproc": { "color": "$sk_preproc" },
          "primary": { "color": "${C[text]}" },
          "property": { "color": "$sk_prop" },
          "punctuation": { "color": "$sk_punc" },
          "punctuation.bracket": { "color": "$sk_punc" },
          "punctuation.delimiter": { "color": "$sk_punc" },
          "punctuation.list_marker": { "color": "${C[blue]}" },
          "punctuation.special": { "color": "$sk_spec" },
          "string": { "color": "$sk_str" },
          "string.escape": { "color": "$sk_esc" },
          "string.regex": { "color": "${C[red]}" },
          "string.special": { "color": "$sk_spec" },
          "string.special.symbol": { "color": "$sk_esc" },
          "tag": { "color": "$sk_tag" },
          "text.literal": { "color": "$sk_str" },
          "title": { "color": "${C[accent]}", "font_weight": 700 },
          "type": { "color": "$sk_type" },
          "type.builtin": { "color": "$sk_const" },
          "type.super": { "color": "$sk_type" },
          "variable": { "color": "$sk_var" },
          "variable.builtin": { "color": "$sk_const" },
          "variable.member": { "color": "$sk_var" },
          "variable.parameter": { "color": "$sk_const" },
          "variant": { "color": "$sk_type" }
        }
      }
    }
  ]
...
EOF
  ...
  [ -f "$settings" ] || return
  sed -i "s|\"$mode\": \"[^\"]*\"|\"$mode\": \"$theme_name\"|" "$settings"
